...
if (isset($_POST['reg_user'])) {
    ...
    //     move_uploaded_file($tmp_name, $img_upload_path);
        ...
        $sql = "UPDATE  card_details SET firstname= '$fname', lastname='$lname', email='$email', phone='$phone', caddress='$address', company_name='$cname', psummary='$psummary', jobtitle='$jobtitle', website_link='$website', created_on ='$created_at', updated_at = NOW() WHERE card_id = $card_id ";
        if($db->query($sql) === true){
            $card_id = mysqli_insert_id($db);
            echo "<script>location.href='card_details_view2.php';</script>";
        } else{
